...
namespace deng {
    ...
    void Camera::setCameraViewRotation() {
        ...
        if(this->m_mouse_yPos > (this->m_mouse_sens / 2)) {
            this->m_mouse_yPos = this->m_mouse_sens / 2;
            glfwSetCursorPos(this->m_window->getWindow(), this->m_mouse_xPos * this->m_window->getSize().x, (this->m_mouse_sens / 2) * this->m_window->getSize().y);
        }
        else if(this->m_mouse_yPos < -(this->m_mouse_sens / 2)) {
            this->m_mouse_yPos = -(this->m_mouse_sens / 2);
            glfwSetCursorPos(this->m_window->getWindow(), this->m_mouse_xPos * this->m_window->getSize().x, -(this->m_mouse_sens / 2) * this->m_window->getSize().y);
        }
        ...
        // this->view_matrix.updateSideCoords(getFractionNumerator(this->m_mouse_yPos, (this->m_mouse_sens / 2), -180.0f), getFractionNumerator(this->m_mouse_xPos, this->m_mouse_sens, 360.0f));
        this->view_matrix.setTransformationMatrix();
        this->view_matrix.setRotation(getFractionNumerator(this->m_mouse_yPos, (this->m_mouse_sens / 2), -180.0f), getFractionNumerator(this->m_mouse_xPos, this->m_mouse_sens, 360.0f));
    }
