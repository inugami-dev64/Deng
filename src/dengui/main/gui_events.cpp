...
namespace dengui {
    ...
    bool Events::findCollision(dengMath::vec2<float> point, WindowElement *p_elem) {
    ...
    void Events::pushWindowElements(std::vector<WindowElement> elems) {
        ...
        for(r_index = 0; r_index < elems.size(); l_index++, r_index++) {
            ...
            (*m_info.p_assets)[l_index].indices.p_indices = (uint32_t*) calloc (
            ...
            memcpy (
                ...
                elems[r_index].indices.size() * sizeof(uint32_t)
            );
            ...
            case ELEMENT_COLOR_MODE_TEXTURE_MAPPED:
                ...
                // Data copy
                (*m_info.p_textures)[tex_index].texture.pixel_data.p_pixel_data = elems[r_index].texture.data();
