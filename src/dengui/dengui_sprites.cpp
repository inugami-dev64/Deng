    ...
    void ShapeVerticesDataCreator::getRectangleVertices(RectangleInfo *p_rectangle_info, WindowObject *p_window_object) {
        if(p_rectangle_info != nullptr && p_window_object != nullptr) {
            ...
            case DENG_TRUE: {
                ...
                p_rectangle_info->p_indices_data->resize(local_indices_size + 6);
            ...
            case DENG_FALSE: {
                ...
                p_rectangle_info->p_indices_data->resize(local_indices_size + 24);
                ...
                float local_distance_from_corner_point = (p_rectangle_info->border_thickness / sin(deng::degToRad(45)));
                ...
                case DENGUI_NON_FILLED_BORDER_OFFSET_MODE_IN: {
                    for(size_t i = 4, angle = 135; i < 8; i++, angle += 90) {
                        local_positions = deng::getCartesianCoordsPoint({(*p_rectangle_info->p_vertices_data)[local_vertices_size + (i - 4)].position_vec.first, (*p_rectangle_info->p_vertices_data)[local_vertices_size + (i - 4)].position_vec.second}, angle, local_distance_from_corner_point, DENG_TRUE);
