...
namespace dengMath {
     ...
     */
    template<typename T>
    vec4<T> vec4<T>::operator+(const T &c) {
        if(std::is_floating_point<T>::value || std::is_integral<T>::value) {
            vec4<T> out = {
                first + c,
                second + c,
     ...
     */
    template<typename T>
    T vec4<T>::operator*(const vec4<T> &vec) {
        if(std::is_floating_point<T>::value || std::is_integral<T>::value) {
            T out = (
                first * vec.first +
                second * vec.second +
     ...
     */
    template<typename T>
    deng_bool_t vec4<T>::operator==(const vec4<T> &vec) { 
        return (deng_bool_t) (                
            first == vec.first && 
            second == vec.second && 
            third == vec.third && 
     ...
     */
    template<typename T>
    deng_bool_t vec4<T>::operator==(const das_ObjColorData &color) {
        if(std::is_floating_point<T>::value) {
            return (deng_bool_t) (
                first == color.col_r &&
                second == color.col_g &&
     ...
     */
    template<typename T>
    deng_bool_t vec4<T>::operator==(const das_ObjVertData &vert) {
        if(std::is_floating_point<T>::value) {
            return (deng_bool_t) (
                first == vert.vert_x &&
                second == vert.vert_y &&
     ...
     */
    template<typename T>
    deng_bool_t vec4<T>::operator==(const das_ObjNormalData &norm) {
        if(std::is_floating_point<T>::value) {
            return (deng_bool_t) (
                first == norm.nor_x &&
                second == norm.nor_y &&
