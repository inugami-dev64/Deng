...
namespace dengUtils {
    ...
    dengMath::vec2<float*> GridManager::getOrdereredVertexDataElements(const uint32_t index, const deng_CoordinateAxisType &axis) {
    ...
    void GridManager::generateAxisLineVertices(dengMath::vec2<int32_t> &min_vertices, dengMath::vec2<int32_t> &max_vertices, const deng_CoordinateAxisType &axis, int32_t &index) {
        // first is min and second is max
        dengMath::vec2<int32_t*> vertices;
        ...
        for(int32_t line_index = (*vertices.first); line_index < (*vertices.second); line_index++) {
    ...
    void GridManager::generateVertices(dengMath::vec4<float> camera_position) {
        ...
        dengMath::vec2<int32_t> min_vertices; 
        min_vertices.first = static_cast<int32_t>(-this->m_max_vertex_buffer_distance / *this->m_p_grid_width);
        min_vertices.second = static_cast<int32_t>(-this->m_max_vertex_buffer_distance / *this->m_p_grid_lenght);

        dengMath::vec2<int32_t> max_vertices;
        max_vertices.first = static_cast<int32_t>(this->m_max_vertex_buffer_distance / *this->m_p_grid_width);
        max_vertices.second = static_cast<int32_t>(this->m_max_vertex_buffer_distance / *this->m_p_grid_lenght);
