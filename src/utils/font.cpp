...
namespace dengUtils {
    const char *__formats[] = {"otf", "ttf"};

    /* 
     * Verify the font existance in either the locally specified font directory
     * or systems font directory
     */
    deng_bool_t StringRasterizer::__verifyFont (
        ...
        for(l_index = 0; l_index < m_fonts.size(); l_index++) {
            // Find file extension
            ext = cm_ExtractFileExtName((char*) str.font_file);
            fname = cm_ExtractFileName((char*) str.font_file);
            if(ext && fname) {
                // Check if found font format is supported 
                for(size_t i = 0; i < C_ARR_SIZE(__formats); i++) {
                    if(!strcmp(ext, __formats[i])) {
                        is_supported = true;
                        break;
                    }
                }
                ...
                if(!strncmp(fname, str.font_file, strlen(ext)) && is_supported) {
    ...
    void StringRasterizer::__findFontFiles(std::string root_path) {
        ...
        if (root_path != "" && root_path[root_path.size() - 1] != '/')
            ...
            while((contents = readdir(dir))) {
                ...
                {
                case DT_REG:
                    file_ext = cm_ExtractFileExtName(contents->d_name);
                    if
                    (
                        (file_ext && !strcmp(file_ext, "ttf")) || 
                        (file_ext && !strcmp(file_ext, "otf"))
                    ) m_fonts.push_back(root_path + contents->d_name);
                
                    break;

                case DT_DIR:
                    if(std::string(contents->d_name) != "." && std::string(contents->d_name) != "..")
                        __findFontFiles(root_path + contents->d_name);
                    break;
